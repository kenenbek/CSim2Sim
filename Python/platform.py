...
f.write("<platform version=\"4\">\n")
f.write("\n")
f.write("\t<process host=\"Tier0\" function=\"LHCb.SmartFalseTolerant.Scheduler\">\n")
...
for i in range(0, NUMBER_OF_TIERS):
    f.write("\t<process host=\"Tier1_" + str(i + 1) + "\" function=\"LHCb.Dispatcher\">\n")
    f.write("\t\t<argument value=\"" + str(i+1) + "\"/>\n")
    f.write("\t\t<argument value=\"" + str(list_of_nodes[i]) + "\"/>\n")
    f.write("\t</process>\n\n")
    for num in range(1, list_of_nodes[i]+1):
        f.write("\t<process host=\"Tier1_" + str(i+1) + "_" + str(num) + "\" function=\"LHCb.Tier1\">\n")
        f.write("\t\t<argument value=\"" + str(i+1) + "\"/>\n")
